...
<div class="body">
    ...
    <div class="content">
        <form action="/password/find-by-email.do" method="post" class="pure-form pure-form-aligned">
            <fieldset>
                <div class="pure-controls">
                    <input onfocus="$.v.showMessage(findPassword,this);"
                           onblur="$.v.isEmail(findPassword,this);" placeholder="请输入验证邮箱" class="pure-input-1-2"
                           name="email" id="txtValidateEmail"/>
                    <j:span id="errorValidateEmail" cssClass="prompt" name="login_user_name"/>
                </div>


                <div class="pure-controls">

                    <input onclick="$.v.getValidateResult(findPassword);"
                           type="button"
                           class="pure-button pure-button-primary button-xlarge pure-input-1-4" value="找回"/>
                </div>
            </fieldset>
        </form>
    </div>
